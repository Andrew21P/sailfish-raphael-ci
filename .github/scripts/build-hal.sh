...
# Build systemimage for APEX bionic libs
echo "=== Building systemimage for APEX ==="
make -j$(nproc --all) systemimage 2>/dev/null || true

# Debug output
OUT_DIR="$ANDROID_ROOT/out/target/product/$DEVICE"
echo "=== OUT directory contents ==="
ls -la "$OUT_DIR/" || true
echo "=== Checking for APEX/bionic ==="
find "$OUT_DIR" -name "libc.so" 2>/dev/null | head -10 || true
find "$OUT_DIR" -path "*/apex/*" -name "*.so" 2>/dev/null | head -20 || true
